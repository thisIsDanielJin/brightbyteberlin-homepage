  ...
  const items = {
    ...
    app: (
      <div style={{ width: "100%", height: "100%", background: "linear-gradient(160deg, #1E1A2E 0%, #2C2040 100%)", position: "relative", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
        {/* Grid texture - behind the panel */}
        <div style={{ position: "absolute", inset: 0, backgroundImage: "linear-gradient(rgba(196,173,207,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(196,173,207,0.04) 1px, transparent 1px)", backgroundSize: "20px 20px", zIndex: 0 }} />
        <div style={{ width: 224, height: 108, borderRadius: 8, overflow: "hidden", border: "1px solid rgba(196,173,207,0.2)", boxShadow: "0 8px 24px rgba(0,0,0,0.4)", position: "relative", zIndex: 1, background: "#1E1A2E" }}>
        ...
        <div style={{ width: 224, height: 108, borderRadius: 8, overflow: "hidden", border: "1px solid rgba(107,57,119,0.3)", background: "rgba(20,13,24,0.85)", boxShadow: "0 8px 24px rgba(0,0,0,0.5), 0 0 40px rgba(107,57,119,0.1)", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ height: "calc(100% - 22px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 10 }}>
            <svg width="100%" height="100%" viewBox="0 0 200 72" preserveAspectRatio="xMidYMid meet">
              ...
              {/* Inner chip pattern - 3x3 grid of processing units */}
              {[0, 1, 2].map((row) =>
                [0, 1, 2].map((col) => (
                  <rect
                    key={`c${row}${col}`}
              ...
              {/* Active pulse on center */}
              <rect x={94} y={34} width={14} height={6} rx={1.5} fill={C.accent} opacity={0.9}>
